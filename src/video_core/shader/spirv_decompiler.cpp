...
public:
    explicit SPIRVDecompiler(const DeviceSettings& deviceSettings, const ShaderIR& ir,
                             ShaderType stage, const Registry& registry,
                             const Specialization& specialization)
        : Module(0x00010300), ir{ir}, stage{stage}, header{ir.GetHeader()}, registry{registry},
          deviceSettings{deviceSettings}, specialization{specialization} {
        ...
        if (!transform_feedback.empty()) {
            if (deviceSettings.IsExtTransformFeedbackSupported) {
        ...
        if (ir.UsesLayer() || ir.UsesViewportIndex()) {
            ...
            if (stage != ShaderType::Geometry &&
                deviceSettings.IsExtShaderViewportIndexLayerSupported) {
                AddExtension("SPV_EXT_shader_viewport_index_layer");
                AddCapability(spv::Capability::ShaderViewportIndexLayerEXT);
            }
        }

        if (deviceSettings.IsFormatlessImageLoadSupported) {
            AddCapability(spv::Capability::StorageImageReadWithoutFormat);
        }

        if (deviceSettings.IsFloat16Supported) {
            AddCapability(spv::Capability::Float16);
        }
        t_scalar_half = Name(TypeFloat(deviceSettings.IsFloat16Supported ? 16 : 32), "scalar_half");
...
private:
    Id Decompile() {
        ...
        binding = DeclareSampledImages(binding);
...
private:
    ...
    void DeclareLocalMemory() {
        // TODO(Rodrigo): Unstub kernel local memory size and pass it from a
        // register at specialization time.
    ...
    void DeclareInputAttributes() {
        if (!specialization.custom_input_varyings.empty()) {
            if (stage != ShaderType::Fragment) {
                UNREACHABLE_MSG("Custom input varyings may only be defined in a fragment shader");
            }
            for (const auto value : specialization.custom_input_varyings) {
                auto index = static_cast<Attribute::Index>(
                    value + static_cast<u64>(Attribute::Index::Attribute_0));
                ...
                // no decoration equals perspective
                // TODO: is perspective varying interpolation always correct?
    ...
    void DeclareOutputAttribute(Attribute::Index index) {
        ...
        while (element < 4) {
            ...
            if (IsOutputAttributeArray()) {
                ...
                /*if (deviceSettings.GetDriverID !=
                    VK_DRIVER_ID_INTEL_PROPRIETARY_WINDOWS_KHR) {*/
                // Intel's proprietary driver fails to setup defaults for arrayed
                // output attributes.
            ...
            if (tfb && deviceSettings.IsExtTransformFeedbackSupported) {
    ...
    u32 DeclareConstantBuffers(u32 binding) {
        for (const auto& [index, size] : ir.GetConstantBuffers()) {
            const Id type = t_cbuf_std140_ubo;
            const Id id = OpVariable(type, spv::StorageClass::StorageBuffer);
    ...
    u32 DeclareSamplers(u32 binding) {
        for (const auto& sampler : ir.GetSamplers()) {
            ...
            const Id image_type = TypeSampler();
            const Id sampler_pointer_type =
                TypePointer(spv::StorageClass::UniformConstant, image_type);
            const Id pointer_type = TypePointer(spv::StorageClass::UniformConstant, image_type);
            const Id id = OpVariable(pointer_type, spv::StorageClass::UniformConstant);
            AddGlobalVariable(Name(id, fmt::format("sampler_{}", sampler.index)));
            Decorate(id, spv::Decoration::Binding, binding++);
            ...
            samplers.emplace(sampler.index,
                             StandaloneSampler{image_type, sampler_pointer_type, id});
        }
        return binding;
    }

    u32 DeclareSampledImages(u32 binding) {
        for (const auto& sampler : ir.GetSamplers()) {
            if (sampler.is_buffer) {
                continue;
            }
            ...
            const Id type = sampler.is_indexed
                                ? TypeArray(image_type, Constant(t_uint, sampler.size))
                                : image_type;
            const Id pointer_type = TypePointer(spv::StorageClass::UniformConstant, type);
            const Id id = OpVariable(pointer_type, spv::StorageClass::UniformConstant);
            AddGlobalVariable(Name(id, fmt::format("texture_{}", sampler.index)));
            Decorate(id, spv::Decoration::Binding, binding++);
            Decorate(id, spv::Decoration::DescriptorSet, DESCRIPTOR_SET);

            sampled_images.emplace(sampler.index,
                                   SampledImage{image_type, image_type, sampler_pointer_type, id});
        }
        return binding;
    }
        ...
        if (ir.UsesLayer()) {
            if (stage != ShaderType::Vertex ||
                ...
                LOG_ERROR(Render_Vulkan,
                          "Shader requires Layer but it's not supported on this stage "
                          "with this device.");
            }
        }
    ...
    Expression Visit(const Node& node) {
        ...
        if (const auto abuf = std::get_if<AbufNode>(&*node)) {
            ...
            case Attribute::Index::TessCoordInstanceIDVertexID:
                // TODO(Subv): Find out what the values are for the first two elements
                // when inside a vertex shader, and what's the value of the fourth
                // element when inside a Tess Eval shader.
                switch (element) {
                case 0:
                case 1:
            ...
            default:
                ...
                if (!IsAttributeEnabled(location)) {
                    // Disabled attributes (also known as constant attributes) always
                    // return zero.
                    return {v_float_zero, Type::Float};
                }
                const auto type_descriptor = GetAttributeType(location);
        ...
        if (const auto cbuf = std::get_if<CbufNode>(&*node)) {
            ...
            pointer =
                OpAccessChain(t_cbuf_float, buffer_id, v_uint_zero, buffer_index, buffer_element);
        ...
        if (const auto conditional = std::get_if<ConditionalNode>(&*node)) {
            ...
            // It's invalid to call conditional on nested nodes, use an operation
            // instead
    ...
    Expression Assign(Operation operation) {
        ...
        if (const auto gpr = std::get_if<GprNode>(&*dest)) {
            if (gpr->GetIndex() == Register::ZeroIndex) {
                // Writing to Register::ZeroIndex is a no op but we still have to visit
                // its source because it might have side effects.
                Visit(src);
                return {};
            }
    ...
    Expression HNegate(Operation operation) {
        const bool is_f16 = deviceSettings.IsFloat16Supported;
    ...
    Id GetSampler(Operation operation) {
        const auto& meta = std::get<MetaTexture>(operation.GetMeta());
        ASSERT(!meta.sampler.is_buffer);

        const auto& entry = samplers.at(meta.sampler.index);
        Id sampler = entry.variable;
        return OpLoad(entry.sampler_type, sampler);
    }

    Id GetTextureSampledImage(Operation operation) {
        const auto& meta = std::get<MetaTexture>(operation.GetMeta());
        ASSERT(!meta.sampler.is_buffer);
    ...
    Id GetTextureImage(Operation operation) {
        ...
        if (meta.sampler.is_buffer) {
            ...
            const auto& entry = sampled_images.at(meta.sampler.index);
            const Id sampler = OpLoad(samplers.at(meta.sampler.index).sampler_type,
                                      samplers.at(meta.sampler.index).variable);
            const Id image = OpLoad(entry.image_type, entry.variable);
            return OpSampledImage(TypeSampledImage(entry.sampler_type), image, sampler);
        }
    }
    ...
    Expression Texture(Operation operation) {
        ...
        const Id sampled_texture = GetTextureImage(operation);
        ...
        if (meta.depth_compare) {
            ...
            if (can_implicit) {
                return {OpImageSampleDrefImplicitLod(t_float, sampled_texture, coords, dref, mask,
                                                     operands),
                        Type::Float};
            } else {
                return {OpImageSampleDrefExplicitLod(t_float, sampled_texture, coords, dref, mask,
                                                     operands),
                        Type::Float};
            }
        }
        ...
        Id texture;
        if (can_implicit) {
            texture = OpImageSampleImplicitLod(t_float4, sampled_texture, coords, mask, operands);
        } else {
            texture = OpImageSampleExplicitLod(t_float4, sampled_texture, coords, mask, operands);
        }
    ...
    Expression TextureLod(Operation operation) {
        ...
        const Id sampler = GetTextureSampledImage(operation);
        const Id coords = GetCoordinates(operation, Type::Float);
    ...
    Expression TextureGather(Operation operation) {
        ...
        Id texture{};
        if (meta.sampler.is_shadow) {
            texture = OpImageDrefGather(t_float4, GetTextureSampledImage(operation), coords,
            ...
            texture = OpImageGather(t_float4, GetTextureSampledImage(operation), coords,
    ...
    Expression TextureQueryLod(Operation operation) {
        ...
        const auto sampler_id = GetTextureSampledImage(operation);
    ...
    void PreExit() {
        ...
        if (stage == ShaderType::Fragment) {
            ...
            // Write the color outputs using the data in the shader registers,
            // disabled rendertargets/components are skipped in the register
            // assignment.
            u32 current_reg = 0;
            for (u32 rt = 0; rt < Maxwell::NumRenderTargets; ++rt) {
                // TODO(Subv): Figure out how dual-source blending is configured in the
                // Switch.
                for (u32 component = 0; component < 4; ++component) {
                    if (!header.ps.IsColorComponentOutputEnabled(rt, component)) {
                        continue;
            ...
            if (header.ps.omap.depth) {
                // The depth output is always 2 registers after the last color output,
                // and current_reg already contains one past the last color register.
    ...
    Expression BallotThread(Operation operation) {
        ...
        if (!deviceSettings.IsWarpSizePotentiallyBiggerThanGuest) {
        ...
        // For instance a device with a warp size of 64 will return the upper uint
        // when the current thread is 38.
    ...
    std::tuple<Id, Id> CreateFlowStack() {
        // TODO(Rodrigo): Figure out the actual depth of the flow stack, for now it
    ...
    const Id t_cbuf_float = TypePointer(spv::StorageClass::StorageBuffer, t_float);
    ...
    const Id t_cbuf_std140_ubo =
        TypePointer(spv::StorageClass::StorageBuffer, t_cbuf_std140_struct);
    const Id t_cbuf_scalar_ubo =
        TypePointer(spv::StorageClass::StorageBuffer, t_cbuf_scalar_struct);
    ...
    // Nvidia uses these defaults for varyings (e.g. position and generic
    // attributes)
    ...
    std::map<u32, StandaloneSampler> samplers;
...
std::vector<u32> Decompile(const DeviceSettings& deviceSettings,
                           const VideoCommon::Shader::ShaderIR& ir, ShaderType stage,
                           const VideoCommon::Shader::Registry& registry,
                           const Specialization& specialization) {
    return SPIRVDecompiler(deviceSettings, ir, stage, registry, specialization).Assemble();
}
